...
async function migrateData() {
  try {
    ...
    const scheduledPosts = sqlite.prepare('SELECT * FROM scheduled_posts WHERE status != "cancelled"').all();
    
    for (const sp of scheduledPosts) {
      const scheduledTime = convertDate(sp.scheduled_time);
      if (!scheduledTime) {
        console.log(`  ⚠️  Skipping scheduled post ${sp.id} - invalid scheduled time`);
        continue;
      }
