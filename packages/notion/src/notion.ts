...
export const transcripts = {
	getRaw: async (
		...
		const result = await queryDatabase(client, config.transcriptsDb, {
			property: "Status",
			select: {
				equals: "Raw",
			},
...
export const insights = {
	getNeedsReview: async (
		...
		const insightPages = result.data.map((page: any) => ({
			...
			verbatimQuote:
				page.properties["Verbatim Quote"]?.rich_text?.[0]?.plain_text || "",
		}));

		return { success: true, data: insightPages };
	},

	getReadyForPosts: async (
		client: Client,
		config: NotionConfig,
	): Promise<Result<InsightPage[]>> => {
		const result = await queryDatabase(
			...
			await client.pages.create({
				...
				children: [
					...
					},
					{
						...
						heading_2: {
							rich_text: [
								{ type: "text", text: { content: "Verbatim Quote" } },
							],
						},
					},
					{
	...
	getAll: async (
		...
		const result = await queryDatabase(
			client,
			config.insightsDb,
			undefined, // No filter - get all insights
			[
				{
					property: "Score",
					direction: "descending" as const,
...
export const posts = {
	getNeedsReview: async (
		...
		for (const page of result.data) {
			...
			const post: PostPage = {
				id: page.id,
				title:
					pageData.properties.Title?.title?.[0]?.plain_text || "Untitled Post",
	...
	getScheduled: async (
		...
		const postPages = result.data.map((page: any) => ({
			...
			createdTime: page.created_time,
			scheduledDate:
				page.properties["Scheduled Date"]?.date?.start || undefined,
			content: "", // Will be populated separately
		}));
	...
	getAll: async (
		...
		const postPages = result.data.map((page: any) => ({
			...
			createdTime: page.created_time,
			sourceInsightId:
				page.properties["Source Insight"]?.relation?.[0]?.id || "",
	...
	getApproved: async (
		...
		const result = await queryDatabase(client, config.postsDb, finalFilter, [
			{
				property: "Created time",
				direction: "ascending" as const,
			},
...
export const cleanedTranscripts = {
	getReadyForProcessing: async (
		client: Client,
		config: NotionConfig,
	): Promise<Result<CleanedTranscriptPage[]>> => {
		const result = await queryDatabase(client, config.cleanedTranscriptsDb, {
			property: "Status",
			select: {
				equals: "Ready",
			},
		...
		const cleanedPages = result.data.map((page: any) => ({
			...
			status: page.properties.Status?.select?.name || "Unknown",
			sourceTranscriptId:
				page.properties["Source Transcript"]?.relation?.[0]?.id || "",
			createdTime: page.created_time,
		}));
