...
export class ScheduledPostRepository extends BaseRepository {
   ...
   */
  async create(data: Partial<NewScheduledPost> & { 
    platform: 'linkedin' | 'x'; 
    content: string; 
    scheduledTime: string 
    ...
    return this.execute(async () => {
      ...
      console.log(`📅 Created scheduled post: ${id} for ${data.platform} at ${data.scheduledTime}`);
   ...
   */
  async update(id: string, data: Partial<NewScheduledPost>): Promise<Result<ScheduledPostView>> {
    return this.execute(async () => {
   ...
   */
  async updateStatus(id: string, status: ScheduledPost['status']): Promise<Result<void>> {
    return this.execute(async () => {
  ...
  async getStats(): Promise<Result<ScheduledPostStats>> {
    return this.execute(async () => {
      ...
      const platformResults = this.sqlite.prepare(
      ...
      const upcoming24h = upcoming24hResult.success ? upcoming24hResult.data.length : 0;

      console.log(`📊 Scheduled post stats - Total: ${total}, By platform:`, byPlatform, `Upcoming 24h: ${upcoming24h}`);
