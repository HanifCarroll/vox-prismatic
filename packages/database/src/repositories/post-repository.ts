...
export class PostRepository extends BaseRepository {
	...
	async findWithRelatedData(filters?: PostFilter): Promise<Result<PostView[]>> {
		return this.execute(async () => {
			...
			if (filters?.insightId) {
				postViews = postViews.filter(
					(post) => post.insightId === filters.insightId,
				);
			}
	...
	async create(
		...
		return this.execute(async () => {
			...
			console.log(
				`📝 Created post: ${id} - "${data.title}" for ${data.platform}`,
			);
			return result.data;
	...
	/**
	 * Get approved posts for scheduler sidebar
	 */
	async getApprovedPostsForScheduler(): Promise<Result<PostView[]>> {
		return this.findWithRelatedData({ status: "approved" });
	}
