...
export const getDatabaseStats = () => {
  ...
  try {
    // Get table counts
    const transcriptCount = database.select().from(schema.transcripts).all().length;
    ...
    const scheduledPostCount = database.select().from(schema.scheduledPosts).all().length;
