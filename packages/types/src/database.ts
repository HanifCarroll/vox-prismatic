...
/**
 * Shared Database Schema for Content Creation Platform
 * Centralized schema definition with proper type inference
 */
...
export const insights = sqliteTable(
	...
	{
		id: text("id").primaryKey(),
		cleanedTranscriptId: text("cleaned_transcript_id")
			.notNull()
	...
	(table) => [
		index("idx_insights_cleaned_transcript_id").on(table.cleanedTranscriptId),
		index("idx_insights_status").on(table.status),
		index("idx_insights_total_score").on(table.totalScore),
...
export const posts = sqliteTable(
	...
	(table) => [
		index("idx_posts_insight_id").on(table.insightId),
		index("idx_posts_platform").on(table.platform),
...
export const scheduledPosts = sqliteTable(
	...
	(table) => [
		index("idx_scheduled_posts_post_id").on(table.postId),
		index("idx_scheduled_posts_platform").on(table.platform),
		index("idx_scheduled_posts_status").on(table.status),
		index("idx_scheduled_posts_scheduled_time").on(table.scheduledTime),
		index("idx_scheduled_posts_created_at").on(table.createdAt),
	],
);
...
export const processingJobs = sqliteTable(
	...
	(table) => [
		index("idx_jobs_type_status").on(table.jobType, table.status),
...
// =====================================================================
// TYPE INFERENCE - Export Drizzle-inferred types
// =====================================================================
