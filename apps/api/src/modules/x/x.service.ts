...
export class XService implements SocialMediaClient {
  ...
  constructor(private configService: ConfigService) {
    this.config = {
      ...
      accessTokenSecret: this.configService.get<string>('X_ACCESS_TOKEN_SECRET'),
    };
   ...
   */
  async refreshAccessToken(refreshToken: string): Promise<Result<XTokenResponse>> {
    try {
      const params = new URLSearchParams({
  ...
  async generateAppOnlyToken(): Promise<Result<string>> {
    try {
      ...
      if (!response.ok) {
        return {
          success: false,
          error: new Error(`Failed to generate app-only token: ${response.statusText}`),
        };
      }
  ...
  async createThread(tweets: string[]): Promise<Result<XTweet[]>> {
    ...
    try {
      ...
      for (const tweetText of tweets) {
        ...
        if (!result.success) {
          return {
            success: false,
            error: new Error(
              `Failed to create thread at tweet ${createdTweets.length + 1}: ${result.error.message}`
            ),
          };
        }
      ...
      this.logger.log(`✅ Thread created successfully with ${createdTweets.length} tweets`);
      return {
        success: true,
        data: createdTweets,
      };
   ...
   * Get user's tweets
   */
  async getTweets(userId?: string, maxResults: number = 20): Promise<Result<XTweet[]>> {
   ...
   */
  async uploadMedia(mediaData: Buffer, mediaType: string): Promise<Result<XMediaUpload>> {
    if (!this._accessToken) {
      return {
        success: false,
        error: new Error('Not authenticated with X'),
