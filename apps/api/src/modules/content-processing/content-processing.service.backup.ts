...
export class ContentProcessingService implements OnModuleInit, OnModuleDestroy {
    ...
    const cleanTranscriptDeps: CleanTranscriptProcessorDependencies = {
      ...
      triggerInsightExtraction: async (transcriptId: string, cleanedContent: string) => {
        ...
        try {
          ...
          // Still allow the extraction to proceed, but log the tracking failure
        }
      },
    };
      ...
      updateTranscriptProcessingStatus: async (transcriptId: string, updates: Prisma.TranscriptUpdateInput) => {
        ...
        if (updates.status === TranscriptStatus.FAILED) {
          ...
          try {
            ...
            // Fallback: directly update the status if state transition fails
            await this.prisma.transcript.update({
              where: { id: transcriptId },
              data: { 
  ...
  /**
   * Validate a state transition before attempting it
   */
  ...
  /**
   * Scheduled task to clean up stale processing jobs
  ...
  /**
   * Scheduled task to retry failed jobs
   * Runs every hour
   */
