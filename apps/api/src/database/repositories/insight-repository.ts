...
export class InsightRepository extends BaseRepository {
	...
	private convertToView(
		insight: Insight & { transcriptTitle?: string | null },
	): InsightView {
		return {
			id: insight.id || "",
	...
	async findWithTranscripts(
		...
		return this.execute(async () => {
			...
			if (filters?.transcriptId) {
				conditions.push(
					eq(insightsTable.cleanedTranscriptId, filters.transcriptId),
				);
			}
			...
			if (filters?.status && filters.status !== "all") {
				if (filters.status === "needs_review") {
					insightViews = insightViews.filter(
						(insight) => insight.status === "needs_review",
					);
				} else {
					insightViews = insightViews.filter(
						(insight) => insight.status === filters.status,
					);
				}
			}
			...
			if (filters?.postType) {
				insightViews = insightViews.filter(
					(insight) => insight.postType === filters.postType,
				);
			}
			...
			if (filters?.minScore !== undefined) {
				insightViews = insightViews.filter(
					(insight) => insight.scores.total >= filters.minScore!,
				);
			}

			if (filters?.maxScore !== undefined) {
				insightViews = insightViews.filter(
					(insight) => insight.scores.total <= filters.maxScore!,
				);
			}
			...
			// Apply sorting
			if (filters?.sortBy) {
				insightViews = this.applySorting(
					insightViews,
					filters.sortBy,
					filters.sortOrder,
				);
	...
	async bulkUpdateStatus(
		...
		return this.execute(async () => {
			...
			for (const id of ids) {
				...
				// Note: bun:sqlite doesn't provide changes count - assuming 1 per update
			updatedCount += 1;
			}

			console.log(
				`📊 Bulk updated ${updatedCount} insights to status: ${status}`,
			);
