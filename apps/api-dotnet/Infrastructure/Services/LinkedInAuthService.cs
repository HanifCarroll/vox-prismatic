...
public class LinkedInAuthService : ILinkedInAuthService
{
    ...
    public async Task<ContentCreation.Api.Features.Common.DTOs.Auth.LinkedInAuthResponse> HandleCallbackAsync(string code, string state)
    {
        try
        {
            var redirectUri = _configuration["ApiKeys:LinkedIn:RedirectUri"] ?? "http://localhost:5001/api/auth/linkedin/callback";
            ...
            return new ContentCreation.Api.Features.Common.DTOs.Auth.LinkedInAuthResponse
            {
                Success = true,
    ...
    public async Task<ContentCreation.Api.Features.Common.DTOs.Auth.LinkedInStatusResponse> GetConnectionStatusAsync()
    {
        try
        {
            var userId = "default"; // In production, get from current user context
            ...
            if (token != null)
            {
                return new ContentCreation.Api.Features.Common.DTOs.Auth.LinkedInStatusResponse
                {
                    IsConnected = true,
                    ProfileId = token.Platform,
                    ProfileName = "LinkedIn User",
