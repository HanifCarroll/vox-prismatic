...
const getTranscriptContent = async (
	...
	try {
		...
		console.log(
			`📝 Extracted ${formatNumber(content.length)} characters of transcript content (${totalBlocks} blocks)`,
		);
...
const generatePostsForInsight = async (
	...
	try {
		...
		display.error(
			`Failed to generate posts for "${insight.title}": ${metrics.error}`,
		);
		return metrics;
	}
};
...
const processBatch = async (
	...
	const batchPromises = batchInsights.map((insight) =>
		generatePostsForInsight(insight, config),
	);
...
export const runPostGenerator = async (
	...
	try {
		...
		console.log(
			`📊 Found ${readyInsights.length} insight${readyInsights.length === 1 ? "" : "s"} ready for post generation`,
		);
		readyInsights.forEach((insight, index) => {
			console.log(
				`  ${index + 1}. "${insight.title}" (Score: ${insight.score})`,
			);
		});
