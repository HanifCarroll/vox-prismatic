import { PublisherService, type PublishingCredentials } from "../../api/src/services/publisher";
...
export class WorkerPublisher {
  ...
  async processScheduledPosts(): Promise<{
    ...
    try {
      ...
      const result = await this.publisherService.processScheduledPosts(this.credentials);
      
      console.log(`📊 [Worker] Processing complete: ${result.successful} successful, ${result.failed} failed`);
