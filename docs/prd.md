...
- Processing: Asynchronous queued jobs (Laravel queue) with realtime progress over WebSockets (Laravel Reverb + Laravel Echo)
...
- Status stream: WebSocket broadcasts on `private-project.{projectId}` carry `{ step, progress }` updates plus completion/failure notifications.
- Idempotent trigger: `POST /api/projects/{id}/process` remains available; if the project is already processing it returns 409, and the client continues listening to the WebSocket stream.
- Optional fallback: Simple status polling for environments where realtime is unavailable
- Timeouts: Job timeout ~10 minutes
...
- Creating a new content project (MVP - transcript only)
  ...
  5) User sees real-time progress via WebSockets on the project detail page
